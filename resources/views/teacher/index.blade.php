                ...
                <div class="container">
                    ...
                    <div class="teacher-profile-right border-0 teacher-profile-right2" style="height: auto;background-color: transparent;border: 0 !important;box-shadow: none !important;">
                            ...
                            <ul class="teacher-icons px-0">
                                ...
                                <li class="zoom">
                                    <button style="min-width: 100px" class="btn btn-danger">
                                        ...
                                        <span>{{-- $teacher->exams()->mainQuestions()->questions()->count() ?? 0 --}} 0</span>
                                    </button>
                                </li>
                            </ul>
                        </div>
                        ...
                        <ul class="main-section__items">
                            <li class="item my-2">
                                <i class="fas fa-language zoom"></i>
                                <span class="teacher__p">لغة التدريس: {{ $teacher->teachingLanguage->local_name }}</span>
                            </li>
                            <li class="item my-2">
                                <i class="fas fa-book zoom"></i>
                                @foreach($teacher->subjects as $subject)
                                    @if($loop->first)
                                        <span class="teacher__p">{{ $subject->name_ar .' - '. $subject->year->name }}</span>
                                        @continue
                                    @endif
                                        <p class="teacher__p mx-4 my-2">{{ $subject->name_ar .' - '. $subject->year->name }}</p>
                                @endforeach
                            </li>
                            <li class="item my-2">
                                <i class="fas fa-map-marker-alt zoom"></i>
                                <span class="teacher__p">{{ $teacher->city->name .' - '. $teacher->city->state->name }}</span>
                            </li>
                            <li class="item my-2">
                                <i class="fas fa-building zoom"></i>
                                <span class="teacher__p">{{ $teacher->school }}</span>
                            </li>
                            <li class="item my-2">
                                <i class="fa-solid fa-phone-flip"></i>
                                @foreach($teacher->phones as $phone)
                                    @if($loop->first)
                                        <span class="teacher__p">{{ $phone->number }}</span>
                                        @continue
                                    @endif
                                    <p class="teacher__p mx-4 my-2">{{ $phone->number }}</p>
                                @endforeach
                            </li>
                            <li class="item my-2">
                                <i class="fa-solid fa-calendar"></i>
                                <span class="teacher__p">{{ $teacher->created_at->format('Y-m-d') }}</span>
                            </li>
                            <li>
                                <i class="fas fa-globe-africa zoom"></i>
                                @foreach($teacher->sites as $website)
                                    @if($loop->first)
                                        <a class="teacher__p text-muted" href=" {{ $website->site }}" target="_blank" class="d-flex">{{ $website->site }}</a>
                                        @continue
                                    @endif

                                    <p class="teacher__p mx-4 my-2">
                                        <a href="{{ $website->site }}" target="_blank" class="d-flex">{{ $website->site }}</a>
                                    </p>
                                @endforeach
                            </li>
                        </ul>
            ...
            <section class="section teacher-profile">
                <div class="container">
                    <div class="row w-100 mx-0 px-0">
                        <div class="{{ $teacher->youtube ? "col-md-6" : "col-md-12" }} teacher-profile-right">
                            <div class="subject-overview__teacher">
                                <h3 class="mt-3">نبذة تعريفية</h3>
                                <p class="mt-4">{{ $teacher->brief }}</p>
                            </div>
                        </div>
                        @if($teacher->youtube)
                            <div class="col-md-6">
                            @php
                                $videoId = null;
                                if (preg_match('%(?:youtube(?:-nocookie)?\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/)([^"&?/\s]{11})%i', $teacher->youtube, $match)) {
                                    $videoId = $match[1];
                                }
                            @endphp
                            <iframe
